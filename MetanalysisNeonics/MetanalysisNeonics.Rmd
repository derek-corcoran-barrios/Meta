...
```{r, echo= FALSE}
All <- rbind(model0, model1, model4, model7, model8, model9, model10, model11)
...
res6 <- rma(yi, vi, data=dat1, refline = 0, method = "HE", mods =~ factor(Functional.group) + factor(AI.type), slab = paste(dat1$Authors, dat1$Year))

model6 <- data.frame(model = "y ~ AI type + Functional group", p_value = res6$QMp, R_squared = round(res6$R2/100,2), AICc = res6$fit.stats$ML[5])
```
